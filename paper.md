...
To be fully cloud optimised - as defined by @Stern22 - an HDF5 file needs to have a contiguous index for each variable, and the chunks for each variable need to be sensibly chosen and broadly contiguous within the file.
...
Metadata can be repacked to the front of the file and variables can be rechunked and made continuous - which is effectively the same process undertaken when HDF5 data is reformatted to other cloud optimised formats.
