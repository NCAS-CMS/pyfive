...
def make_file_hdf5(our_file, vlen_strings):
    with h5py.File(our_file, "w") as hfile:
        ...
        v = hfile.create_dataset(
            "var_len_str_fv", (4,), dtype=dt, fillvalue=b"this really fills the data"
        )
        v[:2] = vlen_strings
...
def make_contiguous_and_chunked_nc(our_file):
    ...
    with nc.Dataset(our_file, "w", format="NETCDF4") as n:
        ...
        # Chunked variable
        months_chunked = n.createVariable(
            "months_chunked",
            str,
            (
                "y",
...
def make_pathological_nc(our_file):
    ...
    months_m = np.ma.array(months, dtype="S7", mask=[0, 1, 0, 0], fill_value=b"")

    numbers = np.array([["one", "two", "three"], ["four", "five", "six"]], dtype="S5")
        ...
        with h5py.File(tfile) as hfile:
            for k, v in validation.items():
                ...
                try:
                    ...
                    assert np.array_equal(hdata.astype(str), pdata.astype(str)), (
                        f"Failed comparison test for {k}"
                    )
