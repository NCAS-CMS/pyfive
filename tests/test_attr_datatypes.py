...
def attr_datatypes_hdf5_2(tmp_path_factory):
    ...
    subprocess.run(
        [sys.executable, MAKE_ATTR_DATATYPES_SCRIPT_2, str(path)], check=True
    )
    return str(path)
...
def test_numeric_scalar_attr_datatypes():
    with pyfive.File(ATTR_DATATYPES_HDF5_FILE) as hfile:
        ...
        assert hfile.attrs["float32_little"] == 123.0
        assert hfile.attrs["float64_little"] == 123.0

        assert hfile.attrs["float32_big"] == 123.0
        assert hfile.attrs["float64_big"] == 123.0
...
def test_attributes_2(attr_datatypes_hdf5_2):
    ...
    with pyfive.File(attr_datatypes_hdf5_2) as ds:
        ...
        assert (
            ds.attrs["unicode_fixed_arrary"]
            == np.array([unicode.encode(), foobar.encode()])
        ).all()
        ...
        assert (
            ds.attrs["ascii_fixed_array"]
            == np.array([ascii.encode(), "foobar".encode()])
        ).all()
