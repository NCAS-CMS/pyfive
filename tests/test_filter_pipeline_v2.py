...
@pytest.mark.parametrize(
    "chunk_size", [None, (10, 10), (20, 20)], ids=lambda x: f"chunk_{x}"
)
@pytest.mark.parametrize(
    "compression", [None, 9, "lzf"], ids=lambda x: f"compression_{x}"
)
...
def test_hdf5_filters(
    modular_tmp_path, generate_data, chunk_size, compression, shuffle, fletcher32
):
    ...
    file_name = (
        modular_tmp_path
        / f"test_{chunk_size}_{compression}_{shuffle}_{fletcher32}.hdf5"
    )
